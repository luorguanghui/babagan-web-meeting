...
生产环境的屏幕共享经 LiveKit SFU 云端转发，云端为阿里云武汉轻量服务器（2 核 2 GiB、200 Mbps 峰值带宽、无 SLA）。实际运行中**云端带宽波动导致接收方画面不稳定**。共享者本机实测具备直连条件：1 Gbps 有线链路、约 100 Mbps 稳定上行、公网 IPv4（117.151.175.232，湖北移动，非 CGNAT 段）。
...
- ICE 凭据经 `GET /api/v1/meetings/:slug/ice-servers` 获取（服务端经 LiveKit Server API `/rtc/ice` 拉取并缓存 1 小时），STUN/TURN 复用现有 LiveKit TURN 服务。
- 共享者对每名观看者各建一条 `RTCPeerConnection`，同一连接发布屏幕视频 + 屏幕音频（音画同步硬约束）。
- 观看者级回退：协商超时 8 秒、ICE failed 或 RTP 失联 5 秒 → 该观看者切 LiveKit 屏幕订阅；存在回退者时共享者才发布 LiveKit 屏幕轨道，双源不并存。
- P2P 码率档位 5/8/10 Mbps（默认 8），按在线观看人数联动建议；SFU 回退档位 10/13/15 Mbps 不变。

## 权限与安全

- 信令握手校验参与者安全 Cookie；服务端强制"仅共享者发 offer、应答仅能回共享者"。
...
- 云端峰值带宽从 16–60 Mbps（屏幕）降至约 1–8 Mbps（语音 + 回退）。
