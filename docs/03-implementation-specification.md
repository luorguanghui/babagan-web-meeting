...
请求浏览器启用 `echoCancellation`、`noiseSuppression` 和 `autoGainControl`。编码由 LiveKit/浏览器协商为 Opus。页面必须为远端音频提供用户手势后的播放恢复机制，以处理自动播放限制。接收端将全部远端 `microphone` 轨道定义为一路“通话音频”，统一应用 0–100% 音量；屏幕音频独立应用 0–100% 音量，并在 P2P、TURN 与 LiveKit 回退之间保持相同设置。两路默认均为 100%，其中共享音频的 100% 指现有 -6 dB 安全衰减后的最大值；设置仅在当前会议页面会话内保留，重新加入或刷新页面后恢复默认值。
