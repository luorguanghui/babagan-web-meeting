...
共享电脑音频在生产中出现"共享方音量很小、接收方却很大"的间歇性过响（Windows 环回捕获发生在系统音量级之前，叠加交接/AGC 瞬态）。观看者的共享音频回放在进入扬声器前经 WebAudio 处理：用户设置的 0–100% 共享音量 × -6 dB 固定安全衰减，再进入压缩/限幅（threshold −20 dB、ratio 10:1、attack 2 ms、release 300 ms），把电脑音频压到语音附近电平并削平瞬态尖峰。共享音量在 P2P、TURN 与 LiveKit 回退时保持不变；共享者本机预览（静音）不经过该链路。全部远端麦克风另行聚合为“通话音频”，由接收端的独立 0–100% 音量控制。
