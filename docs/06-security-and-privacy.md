...
| 媒体窃听 | HTTPS/WSS、ICE、DTLS-SRTP、证书严格校验 |
...
- WebRTC 媒体使用 DTLS-SRTP。首版不是端到端加密：LiveKit 节点是信任边界，必须限制主机管理员权限。
...
产品界面需清楚说明：系统不录制会议，但媒体会经过自托管 LiveKit 服务器实时转发。

## 7. 日志规则

禁止记录：管理密码、会议密码、哈希完整值、Cookie、Authorization、LiveKit Token/Secret、SDP 全文、ICE 凭据、麦克风或屏幕内容。
