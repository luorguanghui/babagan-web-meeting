...
[[ "$(grep -Fc 'NPM_REGISTRY: "${NPM_REGISTRY:-https://registry.npmmirror.com}"' "$compose_file")" == 2 ]] || { echo 'npm mirror must be passed to both application builds' >&2; exit 1; }
...
need "$api_dockerfile" 'ARG NPM_REGISTRY=https://registry.npmmirror.com'
need "$api_dockerfile" 'pnpm config set registry "$NPM_REGISTRY"'
...
need "$api_dockerfile" 'pnpm --filter @meeting/api --prod deploy /opt/api --legacy'
...
need "$web_dockerfile" 'ARG NPM_REGISTRY=https://registry.npmmirror.com'
need "$web_dockerfile" 'pnpm config set registry "$NPM_REGISTRY"'
