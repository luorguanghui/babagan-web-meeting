...
clear_record_variables() {
  unset RECORD_STATE BOOTSTRAP_EMPTY CANDIDATE_SHA STARTED_AT_UTC RELEASE_SHA DEPLOYED_AT_UTC DATABASE_BACKUP DATABASE_BACKUP_SHA256 OVERRIDE_FILE SSH_POLICY IS_BOOTSTRAP_PENDING BOOTSTRAP_VOLUME_NAME BOOTSTRAP_VOLUME_MOUNTPOINT BOOTSTRAP_VOLUME_PROJECT BOOTSTRAP_VOLUME_MARKER_ID
...
parse_protected_release_record() {
  ...
  declare -A allowed=()
  declare -A seen=()
  for key in RECORD_STATE BOOTSTRAP_EMPTY CANDIDATE_SHA STARTED_AT_UTC RELEASE_SHA DEPLOYED_AT_UTC DATABASE_BACKUP DATABASE_BACKUP_SHA256 OVERRIDE_FILE SSH_POLICY BOOTSTRAP_VOLUME_NAME BOOTSTRAP_VOLUME_MOUNTPOINT BOOTSTRAP_VOLUME_PROJECT BOOTSTRAP_VOLUME_MARKER_ID API_IMAGE_TAG API_IMAGE_ID WEB_IMAGE_TAG WEB_IMAGE_ID CADDY_IMAGE_TAG CADDY_IMAGE_ID LIVEKIT_IMAGE_TAG LIVEKIT_IMAGE_ID PREVIOUS_RELEASE_SHA PREVIOUS_API_IMAGE_TAG PREVIOUS_API_IMAGE_ID PREVIOUS_WEB_IMAGE_TAG PREVIOUS_WEB_IMAGE_ID PREVIOUS_CADDY_IMAGE_TAG PREVIOUS_CADDY_IMAGE_ID PREVIOUS_LIVEKIT_IMAGE_TAG PREVIOUS_LIVEKIT_IMAGE_ID; do allowed[$key]=1; done
