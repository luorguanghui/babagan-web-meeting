...
const zhCN: Record<MessageKey, Message> = {
  ...
  'join.nicknameRequired': '请输入昵称。', 'join.failed': '无法加入会议。', 'join.secureRequired': '请通过安全的 HTTPS 连接加入会议。',
  ...
  'room.eyebrow': '会议室', 'room.heading': ({ name }) => `${name}，你已进入会议`, 'room.devicesFailed': '无法获取音频设备列表。',
  ...
  'controls.label': '会议控制', 'controls.connection': ({ state }) => `连接状态：${state}`, 'controls.microphoneStatus': ({ state }) => `麦克风${state}。`,
  ...
  'controls.screenQuality': '共享画质', 'controls.standard': '标准（1080p30）', 'controls.motion': '高动态（1080p60）', 'controls.share': '共享屏幕', 'controls.stopShare': '停止共享屏幕',
  ...
  'screen.stage': '共享屏幕区域', 'screen.empty': '当前没有人共享屏幕。', 'screen.participant': '参会者', 'screen.videoLabel': ({ name }) => `${name} 的共享屏幕`, 'screen.fullscreen': '全屏观看共享屏幕',
