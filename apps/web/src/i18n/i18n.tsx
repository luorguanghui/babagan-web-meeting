...
const en = {
  ...
  'controls.adaptiveQuality': 'Adaptive 1080p · 60 fps',
  ...
  'controls.flow': 'Frame-rate first (720p60)',
  'controls.standard': 'Standard (1080p60)',
  'controls.motion': 'Crisp (1080p60)',
  'controls.screenBitrate': 'Maximum screen-share bitrate',
  'controls.p2pHint': ({ count, bitrate }) => count === 0
    ? 'Direct P2P sharing, up to 5–10 Mbps; boosted 1.5× once direct.'
    : `Direct P2P sharing · suggested ${bitrate} Mbps for ${count} online viewer${count === 1 ? '' : 's'}; boosted 1.5× once direct`,
...
const zhCN: Record<MessageKey, Message> = {
  ...
  'controls.label': '会议控制', 'controls.primaryActions': '主要会议操作', 'controls.settings': '音频与共享设置', 'controls.adaptiveQuality': '自适应 1080p · 60 帧', 'controls.connection': ({ state }) => `连接状态：${state}`, 'controls.microphoneStatus': ({ state }) => `麦克风${state}。`,
  ...
  'host.grant': ({ name }) => `允许 ${name} 共享屏幕`, 'host.kick': ({ name }) => `移出 ${name}`, 'host.end': '结束会议', 'host.confirmEnd': '要为所有人结束会议吗？',
