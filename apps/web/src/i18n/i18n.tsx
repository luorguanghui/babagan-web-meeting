...
const en = {
  ...
  'room.cloudflareUplinkProbing': 'Cloudflare available uplink: probing…',
  'room.cloudflareUplinkReady': ({ bitrate }) => `Cloudflare available uplink: ${bitrate} Mbps (tested)`,
  'room.cloudflareUplinkFailed': 'Cloudflare uplink test unavailable',
...
const zhCN: Record<MessageKey, Message> = {
  ...
  'room.eyebrow': '会议室', 'room.heading': ({ name }) => `${name}，你已进入会议`, 'room.sharingBy': ({ name }) => `正在共享：${name}`, 'room.cloudflareUplinkProbing': 'Cloudflare 可用上行：探测中…', 'room.cloudflareUplinkReady': ({ bitrate }) => `Cloudflare 可用上行：${bitrate} Mbps（测速）`, 'room.cloudflareUplinkFailed': 'Cloudflare 上行测速暂不可用', 'room.sidePanel': '会议侧栏', 'room.management': '会议管理', 'room.devicesFailed': '无法获取音频设备列表。',
