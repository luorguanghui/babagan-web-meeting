...
const en = {
  ...
  'room.sharingBy': ({ name }) => `Now sharing: ${name}`,
  'room.cloudflareTurnProbeProbing': 'Cloudflare TURN path probe: measuring…',
  'room.cloudflareTurnProbeReady': ({ bitrate }) => `Cloudflare TURN path probe: ${bitrate} Mbps`,
  'room.cloudflareTurnProbeRemeasuring': ({ bitrate }) => `Cloudflare TURN path probe: remeasuring (last ${bitrate} Mbps)`,
  'room.cloudflareTurnProbeUnavailable': 'Cloudflare TURN path probe unavailable (does not affect the TURN connection)',
  ...
  'controls.backToMore': 'Back to More',
  ...
  'controls.callAudioShort': 'Call volume',
  'controls.sharedAudioShort': 'Shared volume',
  'controls.volumeQuickMenu': 'Quick audio controls',
  'controls.screenQuality': 'Screen quality',
  'controls.flow': 'Flow (720p30, resolution first)',
  'controls.standard': 'Standard (1080p30, frame rate first)',
  'controls.motion': 'Motion (1080p60, frame rate first)',
  ...
  'controls.screenShareTurnProvider': 'Screen-share TURN provider',
  'controls.screenShareTurnProviderAuto': 'Auto',
  'controls.screenShareTurnProviderCoturn': 'Server coturn',
  'controls.screenShareTurnProviderCloudflare': 'Cloudflare TURN',
  'controls.screenShareTurnProviderHint': 'This provider selection applies to the current share and is controlled by the sharer.',
  ...
  'screenTransport.turnCloudflare': 'TURN relay · Cloudflare',
  'screenTransport.turnCoturn': 'TURN relay · coturn',
  'screenTransport.turnMixed': 'TURN relay · mixed providers',
  ...
  'stats.noData': 'No active screen-share data.',
  'stats.sender': 'Sender',
  ...
  'stats.profileTarget': 'Fixed profile target',
  'stats.transportCap': 'Dynamic transport cap',
  'stats.encoderTarget': 'Encoder target bitrate',
  'stats.actualOutgoing': 'Actual outgoing bitrate',
  'stats.rtcEstimate': 'RTC available estimate',
  'stats.scale': 'Pixel sampling scale',
  'stats.selectedCandidate': 'Selected candidate',
  'stats.selectedCandidateUrl': 'Selected candidate URL',
  'stats.relayProtocol': 'Relay protocol',
  'stats.selectedProvider': 'Selected provider',
...
const zhCN: Record<MessageKey, Message> = {
  ...
  'room.eyebrow': '会议室', 'room.heading': ({ name }) => `${name}，你已进入会议`, 'room.sharingBy': ({ name }) => `正在共享：${name}`, 'room.cloudflareTurnProbeProbing': 'Cloudflare TURN 路径探测：测量中…', 'room.cloudflareTurnProbeReady': ({ bitrate }) => `Cloudflare TURN 路径探测：${bitrate} Mbps`, 'room.cloudflareTurnProbeRemeasuring': ({ bitrate }) => `Cloudflare TURN 路径探测：重测中（上次 ${bitrate} Mbps）`, 'room.cloudflareTurnProbeUnavailable': 'Cloudflare TURN 路径探测暂不可用（不影响 TURN 连接）', 'room.sidePanel': '会议侧栏', 'room.management': '会议管理', 'room.devicesFailed': '无法获取音频设备列表。',
  ...
  'controls.label': '会议控制', 'controls.primaryActions': '主要会议操作', 'controls.primaryToolbar': '主要会议控制', 'controls.more': '更多', 'controls.audioDevices': '音频与设备', 'controls.screenSettings': '屏幕共享设置', 'controls.webrtcData': 'WebRTC 数据', 'controls.settings': '音频与共享设置', 'controls.settingsShort': '设置', 'controls.navigation': '会议导航', 'controls.closePanel': '关闭面板', 'controls.backToMore': '返回更多', 'controls.adaptiveQuality': '自适应屏幕共享 · 30–60 帧', 'controls.connection': ({ state }) => `连接状态：${state}`, 'controls.microphoneStatus': ({ state }) => `麦克风${state}。`,
  ...
  'controls.microphone': '麦克风', 'controls.speakerDevice': '扬声器设备', 'controls.selectSpeaker': '选择扬声器', 'controls.speaker': '扬声器', 'controls.resumeAudio': '点击恢复声音', 'controls.callAudioVolume': '通话音频音量', 'controls.sharedAudioVolume': '共享音频音量', 'controls.callAudioShort': '通话音量', 'controls.sharedAudioShort': '共享音量', 'controls.volumeQuickMenu': '快速音量控制',
  'controls.screenQuality': '共享画质', 'controls.flow': '流畅（720p30，分辨率优先）', 'controls.standard': '标准（1080p30，帧率优先）', 'controls.motion': '动态（1080p60，帧率优先）', 'controls.screenBitrate': '共享最高码率', 'controls.p2pHint': ({ count, bitrate }) => count === 0 ? '请选择每名观看者的 P2P 码率上限；总上行预算将自动保护连接质量。' : `建议每名观看者的 P2P 码率上限为 ${bitrate} Mbps（当前 ${count} 名在线观看者）。`, 'controls.p2pRetry': '重试 P2P 直连', 'controls.viewerTransport': '接收端屏幕传输', 'controls.viewerTransportAuto': '自动（优先 P2P）', 'controls.viewerTransportTurn': 'TURN 中继', 'controls.viewerTransportSfu': 'SFU 中转', 'controls.screenShareTurnProvider': '屏幕共享 TURN 提供方', 'controls.screenShareTurnProviderAuto': '自动', 'controls.screenShareTurnProviderCoturn': '服务器 coturn', 'controls.screenShareTurnProviderCloudflare': 'Cloudflare TURN', 'controls.screenShareTurnProviderHint': '该提供方选择仅作用于当前共享，并由共享者控制。', 'controls.screenCodec': '共享编码', 'controls.codecH264': 'H.264', 'controls.codecAuto': '自动', 'controls.codecVp8': 'VP8', 'screenTransport.p2p': 'P2P 直连', 'screenTransport.turn': 'TURN 中继', 'screenTransport.turnCloudflare': 'TURN 中继 · Cloudflare', 'screenTransport.turnCoturn': 'TURN 中继 · coturn', 'screenTransport.turnMixed': 'TURN 中继 · 混合来源', 'screenTransport.sfu': 'SFU 中转', 'screenTransport.mixed': '混合模式', 'screenTransport.negotiating': '正在建立连接', 'screenTransport.waiting': '等待观看者', 'stats.heading': 'WebRTC 统计', 'stats.requestedCodec': '请求编码', 'stats.collecting': '正在收集统计…', 'stats.noData': '当前没有屏幕共享数据。', 'stats.sender': '发送端', 'stats.receiver': '接收端', 'stats.codec': '协商编码', 'stats.resolution': '分辨率', 'stats.fps': '帧率', 'stats.bitrate': '码率', 'stats.packetLoss': '丢包数', 'stats.rtt': '往返延迟', 'stats.droppedFrames': '丢帧数', 'stats.freezes': '卡顿次数', 'stats.encodeTime': '平均编码耗时', 'stats.jitter': '抖动', 'stats.jitterBuffer': '平均抖动缓冲', 'stats.bandwidth': '可用上行带宽', 'stats.limitation': '质量受限原因', 'stats.turnDiagnostics': 'TURN 路径诊断', 'stats.turnProbeStatus': '探测状态', 'stats.turnProbeCapacity': '已验证 TURN 容量', 'stats.turnProbeSampledAt': '探测采样时间', 'stats.profileTarget': '固定画质目标', 'stats.transportCap': '动态传输上限', 'stats.encoderTarget': '编码器目标码率', 'stats.actualOutgoing': '实际发送码率', 'stats.rtcEstimate': 'RTC 可用估值', 'stats.scale': '像素采样倍数', 'stats.selectedCandidate': '选中候选类型', 'stats.relayProtocol': '中继协议', 'stats.selectedProvider': '选中提供方', 'controls.share': '共享屏幕', 'controls.stopShare': '停止共享屏幕',
