...
export function updateCloudflareEncoding(input: CloudflareEncodingUpdate): CloudflareEncodingState {
  ...
  const currentPathEpoch = finite(measurement.turnProbe.pathEpoch);
  const pathChanged = currentPathEpoch !== undefined
    && previous.lastProbePathEpoch !== undefined
    && currentPathEpoch !== previous.lastProbePathEpoch;
  const probeWindow = readNewProbeWindow(previous, measurement.turnProbe, pathChanged);
  let lowProbeSamples = probeWindow === undefined
    ? (pathChanged ? 0 : previous.lowProbeSamples)
    : probeWindow.measuredBps < currentCap && probeWindow.offeredBps >= currentCap
      ? (pathChanged ? 0 : previous.lowProbeSamples) + 1
      : 0;
  ...
  return {
    ...
    lowProbeSamples,
    ...((probeWindow?.sampledAt ?? (pathChanged ? undefined : previous.lastProbeSampledAt)) === undefined
      ? {}
      : { lastProbeSampledAt: probeWindow?.sampledAt ?? previous.lastProbeSampledAt }),
    ...((currentPathEpoch ?? previous.lastProbePathEpoch) === undefined
      ? {}
      : { lastProbePathEpoch: currentPathEpoch ?? previous.lastProbePathEpoch }),
...
function readNewProbeWindow(
  ...
  turnProbe: TurnPathProbeSnapshot,
  pathChanged = false
): { measuredBps: number; offeredBps: number; sampledAt: number } | undefined {
  if (turnProbe.status !== 'ready' && turnProbe.status !== 'probing') return undefined;
  if (turnProbe.windowKind !== 'verification') return undefined;
  const sampledAt = finite(turnProbe.sampledAt);
  if (sampledAt === undefined || (!pathChanged && sampledAt === previous.lastProbeSampledAt)) return undefined;
  ...
  return measuredBps === undefined || offeredBps === undefined
    ? undefined
    : { measuredBps, offeredBps, sampledAt };
}
