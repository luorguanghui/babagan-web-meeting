...
export interface TurnPathProbeSnapshot {
  ...
  offeredBps?: number;
  /** Calibration climbs the ladder; verification windows may drive control. */
  windowKind?: 'calibration' | 'verification';
  /** Changes whenever the probe is rebuilt onto a new selected path. */
  pathEpoch?: number;
...
export function createTurnProbeCapacityState(pathEpoch?: number): TurnProbeCapacityState {
  return {
    snapshot: {
      status: 'idle',
      probeTargetBps: TURN_PROBE_LADDER_BPS[0],
      ...(pathEpoch === undefined ? {} : { pathEpoch })
    },
...
export function reduceTurnProbeWindow(
  ...
  return {
    snapshot: {
      ...
      windowKind: window.calibration ? 'calibration' : 'verification',
      ...(previous.snapshot.pathEpoch === undefined ? {} : { pathEpoch: previous.snapshot.pathEpoch }),
...
/**
 * Invalidates a selected relay path. The last verified value may remain visible
 * as stale, but no verification window from the old path can validate the new
 * one.
 */
export function invalidateTurnProbePath(
  previous: TurnProbeCapacityState,
  now: number,
  pathEpoch = (previous.snapshot.pathEpoch ?? 0) + 1
): TurnProbeCapacityState {
  const failed = markTurnProbeFailure(previous, now);
  return {
    snapshot: { ...failed.snapshot, pathEpoch },
    recentValidCapacitiesBps: [],
    staleUntil: failed.staleUntil
  };
}
