...
describe('Cloudflare TURN adaptive encoding (fixed target, dynamic cap)', () => {
  ...
  it('never counts a calibration rung as low-capacity control evidence', () => {
    let state = createCloudflareEncodingState(PROFILE_TARGET_BPS);
    for (const sampledAt of [1_000, 2_000, 3_000]) {
      state = step(state, pressureMeasurement({
        offeredBps: 8_000_000,
        measuredCapacityBps: 1_500_000,
        stableCapacityBps: undefined,
        sampledAt,
        status: 'probing',
        windowKind: 'calibration'
      }));
    }

    expect(state.lowProbeSamples).toBe(0);
    expect(state.transportBitrateCapBps).toBe(PROFILE_TARGET_BPS);
  });

  it('consumes the same verification snapshot at most once', () => {
    let state = createCloudflareEncodingState(PROFILE_TARGET_BPS);
    const sameWindow = pressureMeasurement({
      pathEpoch: 1,
      offeredBps: 8_000_000,
      sampledAt: 1_000,
      windowKind: 'verification'
  ...
  it('clears old low-window evidence when the probe path epoch changes', () => {
    const previous: CloudflareEncodingState = {
      ...createCloudflareEncodingState(PROFILE_TARGET_BPS),
      lowProbeSamples: 1,
      bandwidthPressureSamples: 2,
      lastProbeSampledAt: 1_000,
      lastProbePathEpoch: 1
    };

    const state = step(previous, pressureMeasurement({
      pathEpoch: 2,
      offeredBps: 8_000_000,
      sampledAt: 2_000,
      windowKind: 'verification'
