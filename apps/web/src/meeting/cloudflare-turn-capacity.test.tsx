...
describe('TURN probe capacity reducer', () => {
  it('uses confirmed bytes instead of offered bytes', () => {
    ...
    expect(state.snapshot.windowKind).toBe('verification');
  });
  ...
  it('keeps calibration rungs out of stable history until three same-target verification windows agree', () => {
    ...
    expect(state.snapshot.windowKind).toBe('calibration');
  ...
  it('retains the last stable display but requires three fresh windows after path invalidation', () => {
    const ready = probingState([5_500_000, 6_000_000, 6_500_000]);
    let state = invalidateTurnProbePath(ready, 5_000, 2);

    expect(state.snapshot.status).toBe('stale');
    expect(state.snapshot.stableCapacityBps).toBe(6_000_000);
    expect(state.snapshot.pathEpoch).toBe(2);
    expect(state.recentValidCapacitiesBps).toEqual([]);
    expect(state.verificationTargetBps).toBeUndefined();

    for (const sampledAt of [6_000, 7_000]) {
      state = reduceTurnProbeWindow(state, window({
        offeredBps: 12_000_000,
        confirmedBytes: 1_250_000,
        sampledAt
      }));
    }
    expect(state.snapshot.status).toBe('probing');
    ...
    expect(state.snapshot.status).toBe('ready');
    expect(state.snapshot.stableCapacityBps).toBe(10_000_000);
  });
