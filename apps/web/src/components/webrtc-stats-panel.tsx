  ...
  const content = <>
    ...
    {!snapshot?.sender && !snapshot?.receiver
      ...
      : <div className="webrtc-stats-grid">
        ...
        {turnProvider === 'cloudflare' && turnProbe && <TurnDiagnosticsSection snapshot={turnProbe} />}
...
function TurnDiagnosticsSection({ snapshot }: { snapshot: TurnPathProbeSnapshot }) {
  const { t } = useI18n();
  const capacity = snapshot.stableCapacityBps ?? snapshot.measuredCapacityBps;
  const rows: Array<[string, string | undefined]> = [
    [t('stats.turnProbeStatus'), snapshot.status],
    [t('stats.turnProbeCapacity'), bitrate(capacity)],
    [t('stats.turnProbeSampledAt'), snapshot.sampledAt === undefined ? undefined : String(snapshot.sampledAt)],
